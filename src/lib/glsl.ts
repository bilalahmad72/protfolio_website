 ...
 * Shared GLSL chunks.
 *
 * `simplexNoise3D` is the classic Ashima Arts / Stefan Gustavson implementation
 * (MIT licensed, https://github.com/ashima/webgl-noise). It is inlined rather
 * than pulled from a dependency so the shader bundle stays a single module.
 */
...
/** Fresnel term shared by the hero core shaders. */
export const fresnel = /* glsl */ `
float fresnelTerm(vec3 viewDir, vec3 worldNormal, float power) {
  return pow(1.0 - clamp(dot(viewDir, worldNormal), 0.0, 1.0), power);
}
`;
